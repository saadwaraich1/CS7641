...
def processKMandEM(X, y, dataset):
    kMeansSilhouette = []
    kMeansVmeasures = []
    kMeansHomogeneity = []
    kMeansSS = []
    ...
    for clusterSize in clusterSizes:
        ...
        kMeansSilhouette.append(silhouette_score(X=X, labels=yPredicted, metric='euclidean', sample_size=X.shape[0]))
        kMeansSS.append(kMeansModel.score(X))
        ...
        expMaxSilhouette.append(silhouette_score(X=X, labels=yPredicted, metric='euclidean', sample_size=X.shape[0]))
        expMaxLH.append(expMaxModel.score(X))

    saveFigPath = "figures/" + str(dataset) + "-KMEM-RunTimeVsClusterSize.png"
    ...
    saveFigPath = "figures/" + str(dataset) + "-KMeans-SumSquareVsClusterSize.png"
    plot(title="KMeans Sum of Squares vs Cluster Size", xLabel="Cluster Size", yLabel="Sum of Squares", xInput=clusterSizes, \
    yInputs=[kMeansSS], legends=["Sum of square error for each cluster size"], path=saveFigPath)

    saveFigPath = "figures/" + str(dataset) + "-EM-LikelihoodVsClusterSize.png"
    plot(title="Expectation Maximization Log Likelihood vs Cluster Size", xLabel="Cluster Size", yLabel="Log Likelihood", \
    xInput=clusterSizes, yInputs=[expMaxLH], legends=["Log Likelihood for each cluster size"], path=saveFigPath)

    saveFigPath = "figures/" + str(dataset) + "-KMEM-ScoresVsClusterSize.png"
    plot(title="Scores vs Cluster Size", xLabel="Cluster Size", yLabel="Scores", xInput=clusterSizes, \
    ...
    saveFigPath = "figures/" + str(dataset) + "-KMeans-clusters.png"
    ...
    if dataset == "heart":
        ...
        plt.ylabel("BMI")
        plt.scatter(X[:,1], X[:,5], c=yPredicted, alpha = 0.9)
    saveFigPath = "figures/" + str(dataset) + "-EM-clusters.png"
    plt.savefig(saveFigPath)
    plt.clf()
    plt.cla()
    plt.close()
