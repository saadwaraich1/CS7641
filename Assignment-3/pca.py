...
def processPCA(X, y, dataset):

    dPca = PCA()
    fittedPca = dPca.fit_transform(X)
    results = []
    dims = fittedPca.shape[1]

    varianceRatio = dPca.explained_variance_ratio_
    variance = dPca.explained_variance_


    dimensions = [x for x, _ in enumerate(varianceRatio)]
    plt.style.use('seaborn-poster') #sets the size of the charts
    plt.style.use('ggplot')
    plt.title("PCA Variance Ratio")
    plt.xlabel("Features")
    plt.ylabel("Variance Ratio")
    # plt.plot(dimensions, variance, label="Variance")
    plt.bar(x=dimensions, height=varianceRatio, label="Variance Ratio")
    plt.legend(loc='best')
    saveFigPath = "figures/" + str(dataset) + "-PCAVarianceRatio.png"
    ...
    plt.plot(dimensions, variance, label="Variance")
    plt.legend(loc='best')
    saveFigPath = "figures/" + str(dataset) + "-PCAVariance.png"
    plt.savefig(saveFigPath)
    plt.clf()
    plt.cla()
    plt.close()
    ...
    for clusterSize in clusterSizes:
        ...
        yPredicted = kMeansModel.fit_predict(fittedPca)
        endTime = time.time()
        ...
        kMeansSilhouette.append(silhouette_score(X=fittedPca, labels=yPredicted, metric='euclidean', sample_size=X.shape[0]))
        kMeansSS.append(kMeansModel.score(fittedPca))
        ...
        yPredicted = expMaxModel.fit_predict(fittedPca)
        endTime = time.time()
        ...
        expMaxSilhouette.append(silhouette_score(X=fittedPca, labels=yPredicted, metric='euclidean', sample_size=fittedPca.shape[0]))
        expMaxLH.append(expMaxModel.score(fittedPca))
    ...
    saveFigPath = "figures/" + str(dataset) + "-PCA-KMeans-SumSquareVsClusterSize.png"
    plot(title="PCA - KMeans Sum of Squares vs Cluster Size", xLabel="Cluster Size", yLabel="Sum of Squares", xInput=clusterSizes, \
    yInputs=[kMeansSS], legends=["Sum of square error for each cluster size"], path=saveFigPath)

    saveFigPath = "figures/" + str(dataset) + "-PCA-EM-LikelihoodVsClusterSize.png"
    plot(title="PCA - Expectation Maximization Log Likelihood vs Cluster Size", xLabel="Cluster Size", yLabel="Log Likelihood", \
    xInput=clusterSizes, yInputs=[expMaxLH], legends=["Log Likelihood for each cluster size"], path=saveFigPath)

    saveFigPath = "figures/" + str(dataset) + "-PCA-KMEM-ScoresVsClusterSize.png"
    plot(title="PCA - Scores vs Cluster Size", xLabel="Cluster Size", yLabel="Scores", xInput=clusterSizes, \
    ...
    plt.style.use('seaborn-poster') #sets the size of the charts
    plt.style.use('ggplot')
    plt.title("KMeans PCA clusters")
    # fig = plt.figure()
    # ax = Axes3D(fig)
    # ax.scatter(X[:, 1], X[:, 2], X[:, 5],
    #            c=yPredicted, edgecolor='k')
    ...
    saveFigPath = "figures/" + str(dataset) + "-KMeans-PCA-clusters.png"
    plt.savefig(saveFigPath)
    plt.clf()
    plt.cla()
    plt.close()

    print("EM PCA Clustering")
    dPca = PCA()
    fittedPca = dPca.fit_transform(X)
    expMaxModel = GaussianMixture(n_components=2, max_iter=3000, n_init=50)
    yPredicted = expMaxModel.fit_predict(fittedPca)
    plt.style.use('seaborn-poster') #sets the size of the charts
    plt.style.use('ggplot')
    plt.title("Expectation Maximization PCAclusters")
    fig = plt.figure()
    # ax = Axes3D(fig)
    # ax.scatter(X[:, 1], X[:, 2], X[:, 5],
    #            c=yPredicted, edgecolor='k')
    ...
    if dataset == "heart":
        ...
        plt.ylabel("BMI")
        plt.scatter(X[:,1], X[:,5], c=yPredicted, alpha = 0.9)
    saveFigPath = "figures/" + str(dataset) + "-EM-PCA-clusters.png"
    plt.savefig(saveFigPath)
    plt.clf()
    plt.cla()
    plt.close()
