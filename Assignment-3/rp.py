...
def processRP(X, y, dataset):
    ...
    for component in range(1,X.shape[1]):
        ...
        dSp = SparseRandomProjection(n_components=component)
        dSpFitted = dSp.fit_transform(X_train)

        classifier = LinearSVC()
        classifier.fit(dSpFitted, y_train)
        scores.append(metrics.accuracy_score(classifier.predict(dSp.transform(X_test)), y_test))

    # print(components)
    # print(scores)
    saveFigPath = "figures/" + str(dataset) + "-RPAccuracy.png"
    plot("Randomized Projectioins accuracy with LinearSVC", "n_components", "Score", components, [scores], ["Accuracy"], saveFigPath)
    # plotBar("ICA Kurtosis vs Dimension", "Dimension", "Kurtosis", dimensions, results, "figures/ICAKurtosis.png")

    dSp = SparseRandomProjection(n_components=X.shape[1])
    fitteddSp = dSp.fit_transform(X)
    ...
    for clusterSize in clusterSizes:
        ...
        yPredicted = kMeansModel.fit_predict(fitteddSp)
        endTime = time.time()
        ...
        kMeansSilhouette.append(silhouette_score(X=fitteddSp, labels=yPredicted, metric='euclidean', sample_size=X.shape[0]))
        kMeansSS.append(kMeansModel.score(fitteddSp))
        ...
        yPredicted = expMaxModel.fit_predict(fitteddSp)
        endTime = time.time()
        ...
        expMaxSilhouette.append(silhouette_score(X=fitteddSp, labels=yPredicted, metric='euclidean', sample_size=fitteddSp.shape[0]))
        expMaxLH.append(expMaxModel.score(fitteddSp))
    ...
    saveFigPath = "figures/" + str(dataset) + "-SRP-KMeans-SumSquareVsClusterSize.png"
    plot(title="SRP - KMeans Sum of Squares vs Cluster Size", xLabel="Cluster Size", yLabel="Sum of Squares", xInput=clusterSizes, \
    yInputs=[kMeansSS], legends=["Sum of square error for each cluster size"], path=saveFigPath)

    saveFigPath = "figures/" + str(dataset) + "-SRP-EM-LikelihoodVsClusterSize.png"
    plot(title="SRP - Expectation Maximization Log Likelihood vs Cluster Size", xLabel="Cluster Size", yLabel="Log Likelihood", \
    xInput=clusterSizes, yInputs=[expMaxLH], legends=["Log Likelihood for each cluster size"], path=saveFigPath)

    saveFigPath = "figures/" + str(dataset) + "-SRP-KMEM-ScoresVsClusterSize.png"
    plot(title="SRP - Scores vs Cluster Size", xLabel="Cluster Size", yLabel="Scores", xInput=clusterSizes, \
    ...
    saveFigPath = "figures/" + str(dataset) + "-KMeans-RP-clusters.png"
    ...
    plt.style.use('seaborn-poster') #sets the size of the charts
    plt.style.use('ggplot')
    plt.title("Expectation Maximization RP clusters")
    if dataset == "heart":
        plt.xlabel("Resting Blood Pressure")
        plt.ylabel("Serum Cholestoral in mg/dl")
        plt.scatter(X[:,3], X[:,4], c=yPredicted, alpha = 0.9)
        ...
        plt.ylabel("BMI")
        plt.scatter(X[:,1], X[:,5], c=yPredicted, alpha = 0.9)
    saveFigPath = "figures/" + str(dataset) + "-EM-RP-clusters.png"
    plt.savefig(saveFigPath)
    plt.clf()
    plt.cla()
    plt.close()
