...
problem=mlrose.DiscreteOpt(length=50, maximize=True, fitness_fn=mlrose.Knapsack(weights=np.random.uniform(size=length), values=np.arange(1, length+1, 1), max_weight_pct=0.6))
...
titleStr="KnapSack - Mean Fitness Value vs No. of Iterations"
plt.title(titleStr)
...
plt.legend(loc='best')
saveFigPath="figures/" + "KnapSack-fitnesVsIter-" + ".png"
...
for length in lengths:
    ...
    problem=mlrose.DiscreteOpt(length=length, maximize=True, fitness_fn=mlrose.Knapsack(weights=np.random.uniform(size=length), values=np.arange(1, length+1, 1), max_weight_pct=0.6))
...
for length in lengths:
    ...
    for run in range(avgAcross):
        print("Running SA for length: " + str(length) + ", run: " + str(run))
        startTime=time.time()
        bestState, bestFitness, fitnessCurve=mlrose.genetic_alg(problem=problem, curve=True)
        timeTaken=time.time() - startTime
...
for length in lengths:
    ...
    for run in range(avgAcross):
        ...
        startTime=time.time()
        bestState, bestFitness, fitnessCurve=mlrose.mimic(problem=problem, curve=True)
        timeTaken=time.time() - startTime
...
plt.style.use('seaborn-poster')
plt.style.use('ggplot')
titleStr="KnapSack - Mean Fitness Value vs Problem Length"
plt.title(titleStr)
...
plt.legend(loc='best')
saveFigPath="figures/" + "KnapSack-fitnessVal-" + ".png"
plt.savefig(saveFigPath)
plt.clf()
plt.cla()
plt.close()
...
plt.style.use('seaborn-poster')
plt.style.use('ggplot')
titleStr="KnapSack - Mean Run Time vs Problem Length"
plt.title(titleStr)
plt.xlabel("Problem Length")
...
plt.legend(loc='best')
saveFigPath="figures/" + "KnapSack-timeToRun-" + ".png"
plt.savefig(saveFigPath)
plt.clf()
plt.cla()
plt.close()
